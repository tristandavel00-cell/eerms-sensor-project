...
#define SENSOR_ACCEL_ODR_OFF_HZ 0U
#define SENSOR_ACCEL_ODR_MOTION_HZ 12U
#define SENSOR_ACCEL_ODR_MEASUREMENT_HZ 100U
...
static const char *sensor_accel_profile_name(
...
)
{
	...
	if (!device_is_ready(accel_dev)) {
		LOG_ERR("Cannot set accelerometer profile:"
			"device is not ready");
		return -ENODEV;
	}
	...
	switch(profile) {
	case SENSOR_ACCEL_PROFILE_OFF:
		odr_hz = SENSOR_ACCEL_ODR_OFF_HZ;
		break;

	case SENSOR_ACCEL_PROFILE_MOTION:
		odr_hz = SENSOR_ACCEL_ODR_MOTION_HZ;
		break;
	

	case SENSOR_ACCEL_PROFILE_MEASUREMENT:
		odr_hz = SENSOR_ACCEL_ODR_MEASUREMENT_HZ;
		break;

	default:
		LOG_ERR("Invalid accelerometer profile: %d",
			profile);
	...
	LOG_INF("Accelerometer profile changed to %s, ODR=%u Hz",
		sensor_accel_profile_name(profile),
		odr_hz);
...
int sensor_accel_motion_init(
	sensor_accel_motion_callback_t callback,
...
)
{
	...
	if(!device_is_ready(accel_dev)) {
		LOG_ERR("Cannot configure motion detection: "
			"LIS2DW12 is not ready");
		return -ENODEV;
	}

	sensor_ug_to_ms2(
		(int32_t)threshold_mg * 1000,
		&threshold);
	...
	if (ret < 0) {
		LOG_ERR("Failed to configure LIS2DW12"
			"motion threshold: %d",
			ret);
		return ret;
	}
	...
	LOG_INF("LIS2DW12 motion threshold configured: %u mg",
		threshold_mg);
...
int sensor_accel_motion_disarm(void) {
	...
	if (ret < 0) {
		LOG_ERR("Failed to disarm LIS2DW12"
			"motion trigger: %d", ret);
		return ret;
	}
